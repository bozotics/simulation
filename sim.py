...
def rel_vec_to_point(body, point=(0,0), angle=0):
    ...
    """
    absVectAngle = absVect.angle
    #convert Vec2D angle to absolute world angle
    if (absVectAngle >=0 and absVectAngle <= math.pi*0.5) or absVectAngle < 0:
        absVectAngle = math.pi*0.5 - absVectAngle
    else:
        absVectAngle = math.pi*2.5 - absVectAngle
    """
    vect.rotate(angle)
    return vect
...
class MyGame(arcade.Window):
    def __init__(self, width, height, title):
        ...
        self.attackRobot = 0
        self.defenceRobot = 1
        self.robot = [PymunkSprite("images/robot.png", SCREEN_WIDTH/2, SCREEN_HEIGHT*17/40, 0.02175489485134155184916606236403, 2.1),
                      PymunkSprite("images/robot.png", SCREEN_WIDTH/2, SCREEN_HEIGHT/6, 0.02175489485134155184916606236403, 2.1),
    ...
    def on_update(self, delta_time):
        ...
        if self.dribble_state[self.attackRobot] == 0:
            ...
            self.speed[self.attackRobot] = 450
            ...
            if obstaclePos:
                ...
                if self.direction[self.attackRobot] <= math.pi*0.7 or self.direction[self.attackRobot] >= math.pi*1.3:    #front half
                    ...
                    self.chase_state[self.attackRobot] = 1
        ...
        if self.chase_state[self.attackRobot] == 0:
            if self.direction[self.attackRobot] < math.pi:
                ...
                offset = max((self.direction[self.attackRobot]-math.pi*2)*1.0,-math.pi/2)
        ...
        if self.mouseClicked == True:
            ...
            if self.mouseButton == 4:
                ...
                self.robot[2].body.position = self.mousePos
                self.robot[2].body.velocity = (0,0)
    ...
    def on_key_press(self, key, modifiers):
        if key == arcade.key.UP:
            self.target_point_body[1].velocity = (0,500)
        if key == arcade.key.DOWN:
            self.target_point_body[1].velocity = (0,-500)
        if key == arcade.key.RIGHT:
            self.target_point_body[1].velocity = (500,0)
        if key == arcade.key.LEFT:
            self.target_point_body[1].velocity = (-500,0)
        if key == arcade.key.X:
            self.angle[1] = -math.pi*0.5
        if key == arcade.key.Z:
            self.angle[1] = math.pi*0.5
    ...
    def on_key_release(self, key, modifiers):
        if key == arcade.key.Z or key == arcade.key.X:
            ...
            self.target_point_body[1].velocity = (0,0)
